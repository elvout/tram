...
if args.ground_estimate[0] == 0 :
    contact_frames = np.arange(len(video_iterator))
else:
    idx = np.concatenate([np.arange(args.ground_estimate[0]),
                          np.arange(args.ground_estimate[1],0)])
    contact_frames = np.arange(len(video_iterator))[idx]

# Render video
print('Visualize results ...')
visualize_tram(video_iterator, seq_folder, contact_frames=contact_frames, floor_scale=args.floor_scale, bin_size=args.bin_size)
