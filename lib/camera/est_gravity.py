...
def run_spec(bgr_img: np.ndarray):
    ...
    with torch.no_grad():
        ...
        preds = spec(rgb_img, transform_data=True)
        vfov, pitch, roll = preds
        f_pix = rgb_img.shape[0] / (2 * np.tan(vfov / 2.0))
...
def align_cam_to_world(bgr_img: np.ndarray, cam_R, cam_T):
    f_pix, pitch, roll = run_spec(bgr_img)
