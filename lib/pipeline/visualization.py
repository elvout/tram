...
def visualize_tram(
    ...
    view_cam_R = world_cam_R.mT.to(device)
    view_cam_T = -torch.einsum("bij,bj->bi", world_cam_R, world_cam_T).to(device)
    ...
    for i, img in enumerate(tqdm(video_iterator)):
        ...
        if len(verts_list) > 0:
            verts_list = torch.stack(track_verts[i])[:, None].to(device)
            verts_list -= offset
